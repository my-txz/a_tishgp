...
class Config:
    ...
    MIN_MARKET_CAP = 50      # 最小市值 50亿 (剔除微盘股操纵风险)
...
class TechnicalIndicators:
    ...
    def calculate_all(df):
        ...
        包含：MA, MACD, BOLL, ATR, RSI, KDJ, OBV, MFI, 
              新增: CMF (Chaikin), VWAP, Williams %R, BIAS
        """
        df = df.copy()
        ...
        # --- 新增 1: VWAP (成交量加权平均价 - 近似算法) ---
        # 真正的 VWAP 需要分时数据，这里用日线数据模拟一个"中期成本线"
        df['vwap'] = (df['close'] * df['volume']).rolling(20).sum() / df['volume'].rolling(20).sum()
        
        # --- 新增 2: CMF (Chaikin Money Flow) ---
        # 判断资金压力，范围 -0.5 到 0.5
        ...
        for i in range(1, len(df)):
            if flow_change[i] > 0: neg_flow[i] = 0
            else: pos_flow[i] = 0
...
class MarketStructureAnalyzer:
    ...
    def detect_manipulation(df_hist, turnover_rate):
        ...
        # 逻辑：最近3日平均成交量异常大 (>2倍)，但价格涨幅很小 (<1%)
        # 说明有大量资金在流出，但价格被托住（诱多）
        ...
        vol_spike_ratio = recent_vol.mean() / vol_avg_20 if vol_avg_20 > 0 else 1.0
        
        if vol_spike_ratio > 2.0 and price_change_3d < 0.02:
            return "Manipulation: Huge Vol No Price (Distribution)", "Danger"
            
        # 3. CMF (资金流) 背离
        if curr['cmf'] < -0.1 and curr['close'] > curr['ma20']:
...
class AIStrategicAnalyst:
    ...
    def analyze(self, market_env):
        """执行完整分析流程"""
        if not self.fetch_history(): return None
        ...
        # 2. 市场结构与基本面分析
        cap_category, cap_risk_factor = MarketStructureAnalyzer.analyze_market_cap(self.spot.get('总市值', 0))
        turnover = SystemUtils.clean_numeric(self.spot.get('换手率', 0))
        ...
        if pd.isna(curr['macd_slope']) or curr['macd_slope'] <= 0:
            return None
            
        # Screen 2: 回调 (RSI < 65)
        if pd.isna(curr['rsi']) or curr['rsi'] > 65:
            return None
            
        # Screen 3: 动能 (Price Action)
        if curr['close'] < curr['ma5']: return None
        ...
        # 趋势强度 (20分)
        if curr['close'] > curr['ma20'] > curr['ma60'] > curr['ma120']:
            score += 20
            reasons.append("Perfect Uptrend")
        elif curr['close'] > curr['ma20'] > curr['ma60']:
            score += 10
        ...
        if curr['obv'] > curr['obv'].rolling(20).mean().iloc[-1]:
            score += 10
        ...
        if abs(curr['close'] - vwap_val) / vwap_val < 0.05: # 价格贴近成本线
            score += 10
...
class Visualizer:
    ...
    def plot(stock_code, stock_name, data):
        ...
        ax1.plot(df['date'], df['vwap'], label='VWAP (Inst. Cost)', color='cyan', linestyle='--', alpha=0.7)
        ax1.plot(df['date'], df['ma60'], label='MA60 Trend', color='yellow', linestyle=':')
        ...
        ax1.text(df['date'].iloc[0], df['close'].max(), risk_text, color='orange', 
        ...
        ax2.bar(df['date'], df['cmf'], color=['g' if x > 0 else 'r' for x in df['cmf']], alpha=0.6)
        ...
        # --- 子图3：MACD & Volatility ---
        ax3.bar(df['date'], df['macd_hist'], color=['g' if x > 0 else 'r' for x in df['macd_hist']], label='MACD', alpha=0.5)
        ax3.plot(df['date'], df['macd_dif'], label='DIF', color='cyan')
        ...
        file_path = os.path.join(Config.BASE_DIR, f"{stock_code}_{stock_name}_Deep.png")
...
def main():
    ...
    try:
        ...
        spot_df = spot_df[(spot_df['最新价'] > 3) & (spot_df['最新价'] < 300)]
        # 剔除微盘股（市值小于Config设定的值），防止被恶意操纵
        # 注意：这里需要先清洗市值字符串再比较，为简化逻辑放在分析类里做
    ...
    for idx, row in tqdm(spot_df.iterrows(), total=len(spot_df)):
        ...
        try:
            ...
            if res and res['score'] >= 60:
                ...
                res['name'] = name
                res['price'] = row['最新价']
                res['turnover_raw'] = row.get('换手率', 0)
                results.append(res)
    ...
    for stock in tqdm(top_stocks, desc="Generating Viz"):
        try:
            ...
            export_data.append({
                ...
                '核心逻辑': stock['reason'],
                '止损价': round(stock['stop_loss'], 2),
                '目标价': round(stock['take_profit'], 2),
                '走势图': chart_file
            })
        except Exception as e:
